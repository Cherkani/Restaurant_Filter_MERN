...
router.get('/city/:cityId', async (req, res) => {
  try {
    const restaurents = await restaurantService.getRestaurentByCityId(req.params.cityId);
...
router.get('/zones/:zonId', async (req, res) => {
    try {
      const restaurents = await restaurantService.getRestaurentByZoneId(req.params.zonId);
      res.json(restaurents);
    } catch (err) {
      console.error(err.message);
      res.status(500).send('Server Error');
...
router.put('/:id', async (req, res) => {
  try {
    const restaurent = await restaurantService.updateRestaurent(req.params.id, req.body);
    res.json(restaurent);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
